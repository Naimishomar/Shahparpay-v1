...
const SupportWidget = () => {
    ...
    const create = async (event: FormEvent) => {
        ...
        setSaving(true);
        try { const res = await axios.post(api, { subject, description: message }, headers); if (res.data.success) { toast.success('Support ticket created'); setSubject(''); setMessage(''); setShowNew(false); await load(); } }
        catch (error: any) { toast.error(error.response?.data?.message || 'Could not create support ticket'); }
        finally { setSaving(false); }
    };
    const sendReply = async () => { if (!selected || !reply.trim()) return; setSaving(true); try { const res = await axios.post(`${api}/${selected._id}/messages`, { message: reply }, headers); if (res.data.success) { setSelected(res.data.data); setReply(''); await load(); } } catch (error: any) { toast.error(error.response?.data?.message || 'Could not send message'); } finally { setSaving(false); } };
