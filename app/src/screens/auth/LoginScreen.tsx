...
export const LoginScreen: React.FC = () => {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (response.success) {
        login(response.data.token, { ...response.data.user, role: response.data.role });
        setTimeout(() => {
          if (response.data.role === 'admin') {
            navigation.navigate('AdminPortal' as any);
          } else if (response.data.role === 'distributor') {
            navigation.navigate('DistributorPortal' as any);
          } else {
        ...
        >
          ...
          {error && (
            <View style={styles.errorContainer}>
              <MaterialCommunityIcons name="alert-circle" size={20} color="var(--destructive)" />
          ...
          {!isOtpStep ? (
            <View style={styles.form}>
              <Input
                ...
                leftIcon={<Ionicons name="person-outline" size={22} color="var(--muted-foreground)" />}
                autoCapitalize="none"
              ...
              <View style={styles.passwordWrapper}>
                <Input
                  ...
                  leftIcon={<MaterialCommunityIcons name="lock" size={22} color="var(--muted-foreground)" />}
                  rightIcon={
                    <TouchableOpacity onPress={() => setShowPassword(!showPassword)}>
                      {showPassword ? (
                        <MaterialCommunityIcons name="eye-off" size={22} color="var(--muted-foreground)" />
                      ) : (
                        <MaterialCommunityIcons name="eye" size={22} color="var(--muted-foreground)" />
                      )}
                    </TouchableOpacity>
                  }
