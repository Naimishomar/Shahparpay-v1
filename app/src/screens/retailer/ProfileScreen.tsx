...
export const ProfileScreen: React.FC = () => {
  ...
  const menuItems = [
    { name: 'My Profile', icon: 'person', route: 'ProfileDetail' },
      ...
      <View style={styles.profileHeader}>
        <View style={styles.avatarContainer}>
          {user?.profilePicture ? (
            ...
            <View style={[styles.avatarPlaceholder, { backgroundColor: 'var(--primary)' }]}>
              <Text style={styles.avatarInitial}>{user?.name?.charAt(0).toUpperCase() || 'U'}</Text>
            </View>
          )}
        </View>
        <Text style={styles.userName}>{user?.name || 'User'}</Text>
        <Text style={styles.userRole}>{user?.role?.charAt(0).toUpperCase() + user?.role?.slice(1)} Portal</Text>
          ...
          {menuItems.map((item, index) => (
            <TouchableOpacity key={index} style={styles.menuItem} activeOpacity={0.8}>
              <View style={styles.menuIcon}>
                <Ionicons name={item.icon} size={22} color="var(--primary)" />
              </View>
...
const styles = StyleSheet.create({
  ...
  userName: { fontSize: 22, fontWeight: '700', color: 'var(--foreground)' },
  userRole: { fontSize: 14, color: 'var(--muted-foreground)' },
  userId: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 8, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, backgroundColor: 'var(--secondary)' },
  userIdLabel: { fontSize: 11, color: 'var(--muted-foreground)' },
  userIdValue: { fontSize: 12, fontWeight: '600', color: 'var(--foreground)', fontFamily: 'monospace' },
  menuCard: { marginTop: 8 },
  menuItem: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: 'var(--border)' },
  menuIcon: { width: 40, height: 40, borderRadius: 10, backgroundColor: 'var(--primary)', justifyContent: 'center', alignItems: 'center' },
  menuText: { fontSize: 14, fontWeight: '500', color: 'var(--foreground)', flex: 1 },
  walletCard: { marginTop: 8 },
  walletGrid: { flexDirection: 'row', gap: 12 },
  walletItem: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12, borderRadius: 12, backgroundColor: 'var(--secondary)' },
  walletIcon: { width: 40, height: 40, borderRadius: 10, justifyContent: 'center', alignItems: 'center' },
  walletLabel: { fontSize: 11, color: 'var(--muted-foreground)' },
  walletAmount: { fontSize: 16, fontWeight: '700', color: 'var(--foreground)' },
