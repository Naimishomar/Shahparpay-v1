  ...
  try {
    ...
    if (!name || !mobile_no || !email || !product) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: name, mobile_no, email, product',
      });
    }
    ...
    const payload = {
      ...
      redirect_url: process.env.FRONTEND_URL
        ? `${process.env.FRONTEND_URL}/lead-generation`
        : 'https://shahparpay-v1.vercel.app/lead-generation',
    };
    ...
    if (psResponse.success && psResponse.data) {
      ...
      return res.status(400).json({
        success: false,
        message: psResponse.message || 'Failed to generate Lead URL from PaySprint',
      });
    }
  } catch (error) {
...
export const checkStatus = async (req, res) => {
  try {
    ...
    if (psResponse.success && psResponse.data) {
      ...
      return res.status(400).json({
        success: false,
        message: psResponse.message || 'Failed to fetch status from PaySprint',
      });
    }
  } catch (error) {
