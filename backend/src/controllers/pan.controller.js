...
export const setPsaId = async (req, res) => {
  try {
    ...
    if (!txn) {
      return res
        .status(404)
        .json({ success: false, message: 'No PSA registration record found to update' });
    }
...
export const syncPsaStatus = async (req, res) => {
  try {
    ...
    console.log(
      `[PSA Status Sync] Retailer ${retailer._id} manually synced PSA ${psa_id} → ${status}`
    );
...
export const registerBiometricPsa = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: 'Missing required fields for Biometric PSA registration',
      });
    }
    ...
    if (existingTxn && existingTxn.metadata?.psa_id) {
      ...
      if (existingTxn.status === 'REJECTED' || existingTxn.status === 'FAILED') {
        console.log(
          `[Biometric PSA] Previous registration ${existingTxn.metadata.psa_id} was ${existingTxn.status}. Allowing fresh registration.`
        );
      } else {
        return res.status(200).json({
          success: true,
...
export const buyPsaCoupons = async (req, res) => {
  try {
    ...
    if (!psa_id || !amount || Number(amount) <= 0) {
      return res
        .status(400)
        .json({ success: false, message: 'Valid PSA ID and Amount are required' });
    }
    ...
    console.log(
      '[Biometric PSA Payment Request] Provider Response:',
      response.status,
      response.data
    );
...
export const registerStdPsa = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: 'Missing required fields for Standard PSA registration' });
    }
    ...
    if (existingTxn && existingTxn.metadata?.psa_id) {
      if (existingTxn.status === 'REJECTED' || existingTxn.status === 'FAILED') {
        console.log(
          `[Std PSA] Previous registration ${existingTxn.metadata.psa_id} was ${existingTxn.status}. Allowing fresh registration.`
        );
      } else {
        return res.status(200).json({
          success: true,
...
export const updateStdPsa = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: 'Missing required fields for updating Standard PSA registration',
      });
    }
    ...
    if (!existingTxn) {
      return res
        .status(404)
        .json({ success: false, message: 'Standard PSA Registration not found' });
    }

    if (existingTxn.status !== 'REJECTED' && existingTxn.status !== 'FAILED') {
      return res
        .status(400)
        .json({ success: false, message: 'Only rejected applications can be updated.' });
    }
...
export const purchaseStdCoupons = async (req, res) => {
  try {
    ...
    if (!psa_id || !coupon || Number(coupon) <= 0) {
      return res
        .status(400)
        .json({ success: false, message: 'Valid PSA ID and number of coupons are required' });
    }
