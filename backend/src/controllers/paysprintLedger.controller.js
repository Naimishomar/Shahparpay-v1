...
const loginToPortal = async () => {
    ...
    try {
        const response = await axios.post(`${PORTAL_BASE_URL}/login/login`, new URLSearchParams({
        ...
        console.warn("Paysprint portal login failed:", data?.message || data?.statuscode || "unknown error");
        return null;
    } catch (error) {
...
export const getPaysprintCreditLedger = async (req, res) => {
    ...
    if (useDemo) {
        ...
        return res.status(200).json({
            success: true,
            source: 'demo',
            message: "Demo data from the last-week credit ledger export. Configure PAYSPRINT_PORTAL_TOKEN/AUTHKEY in backend/.env for live data.",
            total: rows.length,
    ...
    try {
        ...
        if (!session || !session.token) {
            return res.status(200).json({
                success: true,
                source: 'demo',
                message: "Paysprint portal login failed — showing demo data. Check PAYSPRINT_PORTAL_EMAIL/PASSWORD or PAYSPRINT_PORTAL_TOKEN in backend/.env.",
                total: 0,
                data: []
            });
        }
        ...
        if (data?.status === false && data?.statuscode === 2001) {
            return res.status(200).json({
                success: true,
                source: 'demo',
                message: "Paysprint portal token does not have permission for the credit ledger — showing demo data. Provide a valid PAYSPRINT_PORTAL_TOKEN.",
                total: 0,
                data: []
            });
        }
        ...
        if (!data?.data || !Array.isArray(data.data)) {
            return res.status(502).json({ success: false, message: "Unexpected response from Paysprint ledger API", response: data });
        }
        ...
        return res.status(502).json({ success: false, message: "Failed to fetch Paysprint credit ledger", error: error?.response?.data || error.message });
    }
};
...
export const downloadPaysprintCreditLedger = async (req, res) => {
    ...
    try {
        ...
        const data = response.data;
        if (!data?.data || !Array.isArray(data.data)) {
            return res.status(502).json({ success: false, message: "Unexpected response from Paysprint ledger download", response: data });
        }

        const rows = normalizeResponseRows(data.data);
        return res.status(200).json({ success: true, source: 'live', total: rows.length, data: rows });
    } catch (error) {
        console.error("Paysprint credit ledger download error:", error?.response?.data || error.message);
        return res.status(502).json({ success: false, message: "Failed to download Paysprint credit ledger", error: error?.response?.data || error.message });
    }
};
