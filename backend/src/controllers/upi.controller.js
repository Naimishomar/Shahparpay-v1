...
const isBank6Onboarded = async (merchantCode, mobile) => {
    try {
        const response = await axios.post(getOnboardStatusEndpoint('bank6'), {
    ...
    const claimed = await Transaction.findOneAndUpdate(
        { _id: txn._id, status: { $in: ['PENDING', 'PROCESSING'] } },
        {
            $set: {
                ...
                'metadata.gatewayMessage': gatewayData.message || gatewayData.remarks || txn.metadata?.gatewayMessage || ''
            }
        },
        { new: true }
    );
...
export const generateToken = async (req, res) => {
    try {
        ...
        if (!mobile || !/^\d{10}$/.test(String(mobile))) {
            return res.status(400).json({ success: false, message: "Enter a valid 10-digit customer mobile number" });
        }
        ...
        if (onboarded === false) {
            return res.status(400).json({
                success: false,
                message: "Your merchant is not onboarded for UPI Cashout (Bank 6). Please complete onboarding first."
            });
        }
        ...
        const response = await axios.post(
            `${getUpiBase()}/service/upi/cashout/get_token`,
            payload,
            { headers: getUpiHeaders(), validateStatus: () => true }
        );
        ...
        if (data.response_code === 1 || data.status === true) {
            if (!data.url) {
                await Transaction.findOneAndUpdate(
                    { transactionId: localId },
                    { status: 'FAILED', 'metadata.apiMessage': 'PaySprint returned success without a cashout URL' }
                );
                return res.status(500).json({ success: false, message: "PaySprint returned success without a cashout URL" });
            }
        ...
        return res.status(400).json({ success: false, message: data.message || "Failed to generate UPI cashout token" });
    } catch (error) {
        console.error("UPI Cashout generateToken Error:", error?.response?.data || error.message);
        return res.status(500).json({ success: false, message: "Failed to generate UPI cashout token" });
    }
};
...
export const getTxnStatus = async (req, res) => {
    try {
        ...
        if (!transactionId && !refid) {
            return res.status(400).json({ success: false, message: "transactionId or refid is required" });
        }
        ...
        if (local && ['SUCCESS', 'FAILED'].includes(local.status)) {
            return res.status(200).json({ success: true, data: { status: local.status, transaction: local } });
        }
...
export const webhook = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ status: 200, refid, message: "Transaction completed successfully" });
    } catch (error) {
