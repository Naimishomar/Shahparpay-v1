...
const getEsevaCredentials = () => {
    ...
    if (!partner_unique_id || !secret_key) {
        return { error: "eSevaTech configuration is missing on the server. Please check environment variables." };
    }
...
const applyWalletImpact = async ({ retailer, finalAmount, netCommission, transactionId, type, status, metadata }) => {
...
export const applyPanService = async (req, res) => {
    try {
        ...
        if (!/^[A-Za-z]{5}[0-9]{4}[A-Za-z]{1}$/.test(String(pan_number))) {
            return res.status(400).json({ success: false, message: "Invalid PAN number. Must be 10 characters (e.g. ABCDE1234F)" });
        }
        ...
        console.log(`[eSeva PAN Service] Submitting application for Agent: ${retailer.retailerId}, PAN: ${data.pan_number}`);
        ...
        if (impact.error) {
            ...
            return res.status(200).json({
                ...
                final_amount: finalAmount,
                net_commission: netCommission,
                message: "Application submitted at eSevaTech, but wallet debit failed locally. Please top up your wallet and contact support."
            });
        }
...
export const applyPanCoupon = async (req, res) => {
    try {
        ...
        if (!psa_id || !number_of_coupons) {
            return res.status(400).json({ success: false, message: "psa_id and number_of_coupons are mandatory" });
        }
        ...
        console.log(`[eSeva PAN Coupon] Submitting coupon request for Agent: ${retailer.retailerId}, PSA: ${psa_id}, Qty: ${data.number_of_coupons}`);
        ...
        if (impact.error) {
            return res.status(200).json({
                ...
                final_amount: finalAmount,
                net_commission: netCommission,
                message: "Coupon request submitted at eSevaTech, but wallet debit failed locally. Please top up your wallet and contact support."
            });
        }
