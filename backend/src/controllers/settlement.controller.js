...
const getBankId = (bankName) => {
  ...
  const record = dmtBanks.find(
    (b) => b.BankName && b.BankName.toLowerCase() === bankName.toLowerCase()
  );
...
const extractBeneId = (resData, fallback) => {
  const beneId =
    resData?.bene_id ||
    resData?.data?.beneid ||
    resData?.data?.bene_id ||
    resData?.data?.beneficiary_id;
...
export const getSavedBanks = async (req, res) => {
  try {
    const banks = await BankAccount.find({ userId: req.user.id, deleted: { $ne: true } }).sort({
      createdAt: -1,
    });
  ...
  try {
    ...
    try {
      apiResponse = await axios.post(
        `${baseUrl}/service/payout/payout/list`,
      ...
      return res
        .status(502)
        .json({ success: false, message: 'Failed to fetch accounts from PaySprint' });
    }
    ...
    if (!data || data.status === false) {
      ...
      return res.status(200).json({
        ...
        synced: 0,
        data: [],
        message: data?.message || 'No accounts registered for merchant',
      });
    }
...
export const deleteSettlementBank = async (req, res) => {
  try {
    ...
    if (!bank) {
      return res
        .status(404)
        .json({ success: false, message: 'Bank account not found or unauthorized to delete' });
    }
...
export const addSettlementBank = async (req, res) => {
  try {
    ...
    if (!accountNumber || !ifscCode || !bankName) {
      return res
        .status(400)
        .json({ success: false, message: 'Account number, IFSC, and Bank Name are required' });
    }
    ...
    try {
      ...
      return res
        .status(502)
        .json({ success: false, message: 'Failed to integrate with Payout provider' });
    }
    ...
    const alreadyAdded =
      resData?.response_code === 4 ||
      (resData?.message && /already added/i.test(resData.message || ''));
...
export const getSettlementAccountStatus = async (req, res) => {
  try {
    ...
    if (!bank.beneId) {
      return res.status(400).json({
        success: false,
        message: 'Account has no PaySprint bene_id. Please sync or re-add the account.',
      });
    }
    ...
    try {
      ...
      return res
        .status(502)
        .json({ success: false, message: 'Failed to check account status at PaySprint' });
    }
...
export const uploadSettlementDocument = async (req, res) => {
  try {
    ...
    if (!bank.beneId) {
      return res.status(400).json({
        success: false,
        message: 'Account has no PaySprint bene_id. Please re-add the account first.',
      });
    }
    ...
    if (!passbook) {
      return res.status(400).json({
        success: false,
        message: 'Please upload a passbook / bank statement image (required for all accounts)',
      });
    }
    if (!['image/png', 'image/jpg', 'image/jpeg'].includes(passbook.mimetype)) {
      return res
        .status(400)
        .json({ success: false, message: 'Passbook must be a png/jpg/jpeg image' });
    }
    if (panimage && !['image/png', 'image/jpg', 'image/jpeg'].includes(panimage.mimetype)) {
      return res
        .status(400)
        .json({ success: false, message: 'PAN image must be a png/jpg/jpeg image' });
    }
    if (
      (frontAadhar || backAadhar) &&
      !['image/png', 'image/jpg', 'image/jpeg'].includes((frontAadhar || backAadhar).mimetype)
    ) {
      return res
        .status(400)
        .json({ success: false, message: 'Aadhaar image must be a png/jpg/jpeg image' });
    }

    const form = new FormData();
    form.append('doctype', selectedDoctype);
    form.append('bene_id', String(bank.beneId));
    form.append('passbook', passbook.buffer, {
      filename: passbook.originalname,
      contentType: passbook.mimetype,
    });
    if (panimage)
      form.append('panimage', panimage.buffer, {
        filename: panimage.originalname,
        contentType: panimage.mimetype,
      });
    if (frontAadhar)
      form.append('front_aadhar', frontAadhar.buffer, {
        filename: frontAadhar.originalname,
        contentType: frontAadhar.mimetype,
      });
    if (backAadhar)
      form.append('back_aadhar', backAadhar.buffer, {
        filename: backAadhar.originalname,
        contentType: backAadhar.mimetype,
      });
    ...
    if (resData?.status === false) {
      return res.status(400).json({
        success: false,
        message: resData?.message || 'Failed to upload document',
        data: resData,
      });
    }
    ...
    await BankAccount.updateOne(
      ...
      {
        $set: {
          status: resData?.response_code === 1 ? 'VERIFIED' : 'PENDING',
    ...
    return res.status(200).json({
      success: true,
      message: resData?.message || 'Document uploaded successfully',
      data: resData,
    });
  } catch (error) {
...
const ensureBeneRegistered = async ({
  merchantCode,
  bank,
  accountNumber,
  ifscCode,
...
const executeSettlementTransaction = async ({
  ...
  try {
    const apiResponse = await axios.post(
      `${baseUrl}/service/payout/payout/dotransaction`,
      payload,
      {
        headers: getPaySprintHeaders(),
        validateStatus: () => true,
...
const getSettlementFee = () => {
  ...
  return {
    ...
    gstRate,
    gstAmount,
    total: Math.round((baseCharge + gstAmount) * 100) / 100,
  };
};
...
export const initiateSettlement = async (req, res) => {
  try {
    ...
    if (!isPayoutServiceActive()) {
      return res
        .status(400)
        .json({ success: false, message: getServiceHoursError(), serviceHours: getPayoutWindow() });
    }
    ...
    if (aepsWallet.balance < totalDeduction) {
      return res.status(400).json({
        success: false,
        message: `Insufficient AEPS Wallet balance. (Amount: ₹${amount} + Charge: ₹${fee})`,
      });
    }
    ...
    try {
      ...
      return res.status(400).json({
        success: false,
        message: walletError.message || 'Insufficient AEPS Wallet balance',
      });
    }
    ...
    if (transportError) {
      ...
      if (typeof errorData === 'string' && errorData.includes('<!DOCTYPE html>')) {
        errorMessage =
          'PaySprint Payout API is currently unavailable or the endpoint URL is incorrect (404 Not Found).';
    ...
    const apiMessage =
      resData?.message || (resData?.status ? 'Settlement successful' : 'Settlement failed');
...
export const initiateDirectPayout = async (req, res) => {
  try {
    ...
    if (!accountNumber || !ifscCode || !accountHolderName || !amount || amount <= 0 || !pin) {
      return res
        .status(400)
        .json({ success: false, message: 'Invalid parameters for Direct Payout' });
    }
    ...
    let fee = 0;
    if (mode === 'IMPS') {
      if (amount >= 100 && amount <= 10000) fee = 3.0;
      else if (amount > 10000 && amount <= 25000) fee = 5.0;
      else if (amount > 25000) fee = 8.0;
    }
    ...
    try {
      ...
      return res.status(400).json({
        success: false,
        message: `Insufficient Main Wallet balance. (Amount: ₹${amount} + Fee: ₹${fee})`,
      });
    }

    // Auto-register the beneficiary to obtain a valid bene_id
    let savedBank = await BankAccount.findOne({
      userId: req.user.id,
      accountNumber,
      deleted: { $ne: true },
    });
    ...
    const ackno = resData?.ackno || resData?.data?.ackno || null;
    const apiMessage =
      resData?.message || (resData?.status ? 'Direct payout successful' : 'Direct payout failed');
...
export const checkSettlementStatus = async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(502)
        .json({ success: false, message: 'Failed to fetch status from PaySprint' });
    }
    ...
    if (resData?.status === false) {
      return res.status(400).json({
        success: false,
        message: resData?.message || 'Status enquiry failed',
        data: resData,
      });
    }
    ...
    if (finalStatus === 'FAILED' && (txn.status === 'PROCESSING' || txn.status === 'PENDING')) {
      ...
      await resolveTransaction(
        txn.transactionId,
        'FAILED',
        statusData.status || 'Transaction failed & refunded',
        txn.type === 'DIRECT_PAYOUT' ? 'MAIN' : 'AEPS'
      );
      ...
      await Transaction.updateOne(
        { _id: txn._id },
        {
          $set: {
            status: 'PROCESSING',
            'metadata.apiMessage': statusData.status || 'Transaction is in process',
          },
        }
      );
    }
