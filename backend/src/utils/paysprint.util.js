...
export const isV2OnboardPipe = (pipe) =>
  ONBOARD_V2_PIPES.includes(String(pipe || '').toLowerCase());
...
export const verifyPanDetails = async (merchantcode, name, pan, dob) => {
  try {
    ...
    if (data.status || data.response_code === 1) {
      return {
        success: true,
        message: data.message || 'PAN Verification Successful',
        data: data.data,
      };
    } else {
    ...
    const payload = {
      ...
      callback:
        ...
        (process.env.FRONTEND_URL
          ? `${process.env.FRONTEND_URL}/kyc-status`
          : 'https://your-production-url.com/kyc-status'),
    };
...
export const transferAepsToMainWalletApi = async (merchantcode, amount, referenceid) => {
  try {
    ...
    try {
      ...
      console.error(
        'Non-JSON response from PaySprint Wallet Transfer:',
        responseText.substring(0, 200)
      );
      return {
        success: false,
        message:
          'PaySprint API is currently unavailable or the endpoint URL is incorrect (404 Not Found).',
      };
    }
    ...
    if (data.status || data.response_code === 1) {
      return {
        success: true,
        message: data.message || 'Fund Transfer Successful',
        data: data.data,
      };
    } else {
...
export const fetchMainBalance = async (merchantcode) => {
  try {
    ...
    if (data.status || data.response_code === 1) {
      ...
      if (data.data) {
        balance = parseFloat(
          data.data.balance || data.data.wallet_balance || data.data.cashbalance || 0
        );
      } else {
...
export const fetchAepsBalance = async (merchantcode) => {
  try {
    ...
    if (data.status || data.response_code === 1) {
      ...
      if (data.data) {
        balance = parseFloat(
          data.data.cashbalance || data.data.balance || data.data.wallet_balance || 0
        );
      } else {
